...
`render_denial()` existe desde esta fase (sección 15 exige cobertura total: un motivo de denegación nuevo no puede enviarse sin su mensaje). `render_summary()` llega más adelante dentro de esta misma fase; hasta entonces `update_order_status` persiste `displayed_summary=None` siempre (sección 10). La SPEC 2 conecta `render_summary()` de punta a punta cuando existe el flujo de confirmación fuera de banda (ver SPEC-2 sección 8 y ADR 0002).
